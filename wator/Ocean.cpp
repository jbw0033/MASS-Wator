...
/**
 * callalltest causes all Ocean's to report their location and contents.
 */
void *Ocean::callalltest( void *argument ) {
 ...
 */
void *Ocean::exchangetest( void *argument ) {
  ostringstream convert;
 ...
 */
void *Ocean::checkInMessage( void *argument ) {
...
/**
 */
void *Ocean::printOutMessage( void *argument ) {
  ostringstream convert;
  convert << "printOutMessage Ocean[" << index[0] << "][" << index[1]
 ...
 */
void *Ocean::printShadow( void *argument ) {
  int shadow[4];
  int north[2] = {0, 1};
  int east[2] = {1, 0}; 
  int south[2] = {0, -1};
  ...
  ostringstream convert;
  convert << "printShadow:  Ocean[" << index[0] << "][" << index[1]
...
/** (int)
*	Returns the population of this ocean
*	@return:	AN INT, containing population of local agents of this Ocean
*/
void *Ocean::popCount( void *argument )
{ 
	int *retVal = new int;
	*retVal = agents.size();
	if(agents.size() > 0) {
//		cout << "Fish " << (Agent) agents.at(0)->getId(argument) << " is in ocean " << index[0] << " " << index[1] << endl;
	}
	return retVal;
}
...
*/
void *Ocean::recordAsNeighborPop()
{
...
void *Ocean::resetForNextTurn()
{
	...
	//Debug logging: Expects all ocean are now blind of occupancy of neighbors
	//ostringstream convert;
	//convert << "resetForNextTurn:  Ocean[" << index[0] << "][" << index[1] << "]'s north = " << neighborPop[0] << ", east = " << neighborPop[1] << ", south = " << neighborPop[2] << ", west = " << neighborPop[3];
	//MASS_base::log( convert.str( ) );

//	cout << "resetForNextTurn:  Ocean[" << index[0] << "][" << index[1] << "]'s north = " << neighborPop[0] << ", east = " << neighborPop[1] << ", south = " << neighborPop[2] << ", west = " << neighborPop[3] << endl;
	return NULL;
}

/** (void)
*	Set the outmessage as population of this ocean
*/
void *Ocean::displayPopAsOut()
{
	outMessage_size = sizeof( int );
    outMessage = new int( );
    *(int *)outMessage = (int)agents.size();
	
	//Debug logging: Expected 400 or nxn number of oceans have a single agent on them
	//ostringstream convert;
	//convert << "displayPopAsOut:  Ocean[" << index[0] << "][" << index[1] << "] has the population of " << *((int *)outMessage);
	//MASS_base::log( convert.str( ) );
...
*/
void *Ocean::recordNeighborPopByOut()
{
	//Only need to record neighborpopulation if the ocean has an agent who has delegated the Ocean to do so
	if(agents.size() == 0)
		return NULL;
	...
	ostringstream convert;
	convert << "recordNeighborPopByOut:  Ocean[" << index[0] << "][" << index[1] << "]'s north = " << neighborPop[0] << ", east = " << neighborPop[1] << ", south = " << neighborPop[2] << ", west = " << neighborPop[3];
//	MASS_base::log( convert.str( ) );
	return NULL;
}
...
{
	//Can only set agentID if agent resides on the Ocean
	if(agents.size() == 0)
		return NULL;
	...
	//ostringstream convert;
	//convert << "setMyAgent: Ocean[" << index[0] << "][" << index[1] << "] agent is (" << myAgent << ")";
	//MASS_base::log( convert.str( ) );
	return NULL;
}
...
*	This is so the Nomad can retrieve its next move from the Ocean it is currently on
*/
void *Ocean::getMigrationReservation()
{
	int *retVal;
...
void *Ocean::makeReservation()
{
	...
	//Debug logging: Expects to reserve a space on an unoccupied ocean
	ostringstream convert;
	if(!(nextMove[0] == 0 && nextMove[1] == 0))
		convert << "makeReservation: Ocean[" << index[0] << "][" << index[1] << "] reserving on [" << (index[0] + nextMove[0]) << "][" << (index[1] + nextMove[1]) << "]";
	else
		convert << "makeReservation: Ocean[" << index[0] << "][" << index[1] << "] could not make reservation";
//	MASS_base::log( convert.str( ) );
...
void *Ocean::showReservation()
{
	...
	//Debug logging: Expects to have read a reservation of whichever agentID wishing to migrate to this Ocean or 0 if none
	ostringstream convert;
	convert << "showReservation: Ocean[" << index[0] << "][" << index[1] << "] is reserved (" << *(int *)outMessage << "). InMessage was { " << DEBUG.str() <<"}";
//	MASS_base::log( convert.str( ) );
...
void *Ocean::checkReservation()
{
	...
	ostringstream convert;
	if(reserved)
		convert << "checkReservation: Ocean[" << index[0] << "][" << index[1] << "] successfully reserved on [" << (index[0] + nextMove[0]) << "][" << (index[1] + nextMove[1]) << "]";
	else if(!(nextMove[0] == 0 && nextMove[1] == 0))
		convert << "checkReservation: Ocean[" << index[0] << "][" << index[1] << "] could not reserve on [" << (index[0] + nextMove[0]) << "][" << (index[1] + nextMove[1]) << "]";
	else
		convert << "checkReservation: Ocean[" << index[0] << "][" << index[1] << "] could not decide a reservation.";
//	MASS_base::log( convert.str( ) );
...
*	@post:	Sets the reserve status to true if its reservation is successful
*/
void *Ocean::reservationExchange()
{
	int *retVal = NULL;
	...
	ostringstream convert;
	if(retVal != NULL)
		convert << "reservationExchange: Ocean[" << index[0] << "][" << index[1] << "] reservation message {" << retVal[0] << ", " << retVal[1] << ", " << retVal[2] <<"}";
	else
		convert << "reservationExchange: Ocean[" << index[0] << "][" << index[1] << "] reservation message: No Reservation";
//	MASS_base::log( convert.str( ) );
	return retVal;
}
...
void *Ocean::showReservation2()
{
	...
	//Debug logging: Expects to have read a reservation of whichever agentID wishing to migrate to this Ocean or 0 if none
	ostringstream convert;
	convert << "showReservation2: Ocean[" << index[0] << "][" << index[1] << "] is reserved (" << *(int *)outMessage << ")";
//	MASS_base::log( convert.str( ) );
...
*	Record the outmessage of neighbors as population
*	pre: neighborPop is initialized and contains 4 -1s, outmessage of Oceans are population
*/
void *Ocean::delegateCalculation()
{
	...
	/*
	ostringstream convert;
	convert << "delegateCalculation:  Ocean[" << index[0] << "][" << index[1] << "]'s north = " << neighborPop[0] << ", east = " << neighborPop[1] << ", south = " << neighborPop[2] << ", west = " << neighborPop[3];
	if(!(nextMove[0] == 0 && nextMove[1] == 0))
