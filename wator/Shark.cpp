...
/**
 * initialize a Shark with the given argument.
 */
void *Shark::agentInit( void *argument ) {
  ostringstream convert;
  if(printOutput == true){
//      convert << "agentInit[Shark " << agentId << "] called, argument = " << (char *)argument;
...
*/
const int Shark::cardinals[5][2] = { {0,0}, {1,0}, {0,1}, {-1,0}, {0,-1} };

/**
 * Prints out a message.
 */
void *Shark::somethingFun( void *argument ) {
  ostringstream convert;
  if(printOutput == true){
//      convert << "somethingFun[Shark " << agentId << "] called, argument = " << (char *)argument;
//      MASS_base::log( convert.str( ) );
  }
  return NULL;
}

void *Shark::printCurrentWithID( void *argument) {
	int* turnPtr = (int*)argument;
	ostringstream convert;
//	convert << "Turn " << *turnPtr << " agent Shark " << agentId << " current Location = [" << place->index[0] << "][" << place->index[1] << "]" << endl;
//	MASS_base::log(convert.str());
	return NULL;
}
 ...
 */
void *Shark::createChild( void *argument ) {
//  int nChildren = 0;
//  vector<void *> *arguments = new vector<void *>;
//  arguments->push_back( argument );
//  arguments->push_back( argument );
//  if ( false ) { // only spawn a child if my agent id is even
//    nChildren = 1;
...
//  ostringstream convert;
//  if(printOutput == true){
//      convert << "createChild[Shark " << agentId << "] called to spawn " << nChildren;
////      MASS_base::log( convert.str( ) );
//  }
//  return NULL;
}
 ...
 */
void *Shark::killMe( void *argument ) {
  if ( true) {
    kill( );
  }
  ...
  ostringstream convert;
  if(printOutput == true){
//      convert << "killMe[Shark " << agentId << "] called";
//      MASS_base::log( convert.str( ) );

	  cout << "killMe[Shark " << agentId << "] called";
  }
  return NULL;
}
 ...
 */
void *Shark::callalltest( void *argument ) {
  ostringstream convert;
  if(printOutput == true){
//      convert << "callalltest: agent(Shark " << agentId;
//      MASS_base::log( convert.str( ) );
  }
  ...
  *ret_val = *(int *)argument * 10.0;
  ...
  if(printOutput == true){
      convert.str( "" );
//      convert << "callalltest: agent(Shark " << agentId << "): argument = "
//              << *(int  *)argument
 ...
 */
void *Shark::addData( void *argument ) {
  ...
  ostringstream convert;
  if(printOutput == true){
//      convert << "my shark agent id = " << agentId;
  }
...
int Shark::map( int initPopulation, vector<int> size, vector<int> index, Place* curPlace)
{
	ostringstream convert;
//	if (printOutput) {
//		convert << "Shark inside map function";
//		MASS_base::log(convert.str());
//	}
...
//	if (printOutput == true) {
//		convert.str("");
//		convert << "creating shark agent at " << index[0];
...
*/
void *Shark::calculateMoveset( void *argument ) {
	moveset.clear();
	moveset.push_back(1);
	moveset.push_back(2);
	moveset.push_back(3);
...
*/
void *Shark::delegateMove( void *argument )
{
...
*/
void *Shark::moveDelegated( void *argument )
{
	//Check if iteration to act
	int *partitionCounter = (int*)argument;
	if(iterationToAct != *partitionCounter) {
...
//		convert << "moveDelegated: Shark Agent-" << agentId << " at [" << place->index[0];
//		convert << ", " << place->index[1] << "] to move to [" << x << ", ";
//		convert << y << "] a displacement of [" << nextMove[0] << ", " << nextMove[1] << "]";
	}
	else {
//		convert << "moveDelegated: Shark Agent-" << agentId << " at [" << place->index[0] << ", " << place->index[1] << "] could not move!";
	}
	if(printOutput) { 
//		MASS_base::log( convert.str( ) );
	}
 ...
 */
void *Shark::ravelledxy( void *argument ) {
  int *ret_val = new int;
